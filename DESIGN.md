...
REQ-COM-002 は起動後の通常動作で、ループバックと同一ホスト内を除く外部ネットワーク通信を禁ずる。Compose で可能な範囲として、ユーザ定義ネットワークを `internal: true` にし、実行時は `pull_policy: never` とする。これでコンテナから他ネットワーク（インターネット）への到達は CNI / ブリッジレベルで拒める。ただし次は Compose では表現できない。

- ホストが公開するポート 8000 は inbound であり、遮断対象ではない。
- ホスト OS の NIC 切断、ユーザ名前空間、IPv6、エンジンの DNS フォールバックまでは保証しない。
- rootful と rootless で実装差がある。

主防御はアプリケーションが実行時クライアントを持たないことである。提出時の実証（VER-CON-001）はホスト側でネットワークを遮断して起動コマンドを 1 回実行する。`internal: true` はその補助であり、完全エアギャップの代替ではない。

### 10.2 仕様を満たせなかった箇所

アプリ本体は未実装である。F004 はイメージ定義と Compose 契約までである。Compose の `internal: true` はホスト級の完全遮断ではない（I-NET-001）。実装後に未達があれば本項へ移す。
...
- 実行段階の外部ネットワークは禁止（REQ-CON-001、REQ-COM-002）。Compose の `internal: true` はコンテナ起点の外部到達を拒む補助であり、ホストの完全遮断までは保証しない（I-NET-001）。
