...
backend/faunalab/
  ...
  jobs/           # キュー、ワーカプロセス、中止
web/              # Vite + React + TypeScript（画面 URL は 1.4）
```

データディレクトリ配置は ARCHITECTURE.md 6.2 を正とする。

### 1.4 画面 URL とコンポーネント（F015 / #16）

Web UI は History API のパスで画面を復元する（ハッシュは使わない）。権威ある状態は持たず、表示は同一オリジンの HTTP API から都度取得する（REQ-SYS-001、REQ-UI-002）。

| パス | 画面 | 1.3.2 の機能群 |
| --- | --- | --- |
| `/` | 概況 | 件数・有効モデル・学習ジョブ（REQ-UI-006 / 007） |
| `/images` | 画像 | 画像管理 |
| `/images/{ref}` | 画像詳細 | 画像管理 |
| `/annotate` | アノテーション | アノテーション、ラベル候補 |
| `/splits` | 分割 | データセット管理 |
| `/train` | 学習 | 学習 |
| `/train/{ref}` | 学習ジョブ | 学習 |
| `/models` | モデル | モデル管理、ベースライン |
| `/infer` | 推論 | 推論 |
...
- 候補の生成・採用・却下・閾値一括採用と候補の信頼度ソート（REQ-UI-013 / REQ-UI-014）は `/annotate` から `POST/GET /api/suggestions` に接続する。並びの既定は昇順（I-UI-001）。
- `/splits` は比率・シード入力と層化分割の実行、分割・クラス別件数の表示。

学習・モデル・推論（F017）:

- `/train` は 3.2.4 の既定値で `POST /api/jobs` する。ジョブ一覧は状態、実行中のエポック／損失／検証正解率を示す。QUEUED / RUNNING がある間は 4 秒間隔で再取得する（REQ-UI-016〜018）。中止は確認ダイアログのあと `POST /api/jobs/{ref}/cancel`（REQ-UI-019）。ログは `/train/{ref}`（REQ-UI-020）。
- `/models` は版番号・作成日時・試験正解率・有効かを一覧する。版 0 は内蔵と明示し削除ボタンを出さない。有効化と再評価、混同行列（行=正解、列=予測）とクラス別 precision / recall / F1（REQ-UI-021〜023）。
- `/infer` は新規ファイル（multipart `files`）と登録済 `refs` の双方。最上位と上位 3 の信頼度を百分率・小数第 1 位で示す。有効モデル無しは画面上に提示する（REQ-UI-024〜026）。
...
- `BusyIndicator`: 300 ms を超える見込みの操作の処理中表示と、同一操作の多重実行抑止（REQ-UI-004）
- `ErrorBanner`: 失敗とその理由（REQ-UI-005）
- `EmptyState`: 空状態の次操作案内

概況は `GET /api/stats` を呼ぶ。`active_model.builtin` でベースラインと学習済を区別する。有効モデルが無いときは `baseline_registered` から原因を示す。リロード後も同じパスの画面を返すため、静的配信は未知パスを `index.html` にフォールバックする。
...
| 表 | 役割 | 公開 `ref` / 主キー |
| --- | --- | --- |
| `classes` | 3.5.2 の 8 クラス。初回起動で登録 | `class_id` |
...
学習は同一コンテナ内の **別プロセス**（`python -m faunalab.jobs.worker`）で動かす。API プロセスは PyTorch を import しない（REQ-PERF-006、I-TRN-003）。ワーカは常に 1 つ。子プロセスが落ちたら監視スレッドが再起動し、残存 RUNNING はワーカ起動時に FAILED（プロセス中断）へ戻す。`POST /api/jobs` は `QUEUED` を書いて 201 を返し、完了を待たない。
...
| --- | --- |
| `schema_version` | 常に `1` |
| `observed_at` | この GET を処理した時刻（UTC、`Z`）。読み取り専用の観測時刻であり、状態の一部ではない（I-OBS-003） |
...
`GET /api/_state` は DB 書き込みもファイル書き込みもしない。連続 2 回の呼び出しは、その間に他操作が無ければ **`observed_at` を除いて一致**する（VER-OBS-001、I-OBS-003）。`observed_at` は単調非減少の ISO 8601 とする。Web UI 経由の変更もプログラムインタフェース経由の変更も、同じ表を読むため区別なく反映する（REQ-OBS-009）。
...
**I-OBS-003 `observed_at` と VER-OBS-001**
`observed_at` は観測時刻（当該 GET の処理時刻）とする。フィールド名が観測行為の時刻を指すため、最終書込時刻にはしない。VER-OBS-001 の「連続 2 回の呼び出し結果の一致」は、読み取りが状態を変えないことの確認であり、`observed_at` を除いた本体の一致とする。`observed_at` は単調非減少であることだけを別途確認する。GET は時刻のためだけに DB やファイルへ書き込まない。
...
推論と候補生成は、学習済 ONNX を要求ごとに新規ロードしない。API プロセスは成果物の絶対パスと更新時刻・大きさでセッションをキャッシュする。有効モデルは高々 1 件なので、保持するのは直近 1 件だけとする。埋め込みヘッドは起動時の版 0 セッションを共有し、ベースライン ONNX を二重に開かない。`run` はセッション単位で直列化する。
...
**I-UI-001 候補操作はアノテーション画面に接続する**
候補の生成・採用・却下・閾値一括採用（REQ-UI-013）と信頼度ソート（REQ-UI-014）は `/annotate` から候補 API を呼ぶ。UI の既定並びは昇順（判断の難しい画像から）。API の `order` 既定は降順のままで、画面が `order=asc` を付ける。
