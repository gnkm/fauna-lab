...
本書は、仕様が沈黙している事項（言語、フレームワーク、データの保持、モジュール分割、実行形態）を固定する。観測インタフェース `GET /api/_state` は形式固定。プログラムインタフェースのパス・誤り語彙と JSON フィールドは `openapi.yaml` を正とし、実装と一致させる（F019）。
...
| HTTP API | プログラムインタフェース（3.1.2）。画像・ラベル・分割・学習・モデル・推論・候補ラベル | JSON / マルチパート。OpenAPI 3.1 は `openapi.yaml` を契約とし、実装と一致させる（F019） |
...
プログラムインタフェースのリソース境界・パス・誤り識別子と JSON フィールドは `openapi.yaml` を正とする。実装との一致は F019。
...
JSON フィールドの最終集合は `openapi.yaml` と実装で一致させた（F019）。パス・HTTP メソッド・ページネーション・誤り語彙は F006 以降の契約を維持する。SQLite 表は F007 で DESIGN.md 6.1 に固定し、学習ログ表は F014 で `job_epoch_logs` を足した。
