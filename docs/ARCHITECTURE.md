...
本書は、仕様が沈黙している事項（言語、フレームワーク、データの保持、モジュール分割、実行形態）を固定する。観測インタフェース `GET /api/_state` は形式固定。プログラムインタフェースのパス・誤り語彙は F006 の `openapi.yaml` を正とする。JSON フィールドの最終集合と実装との一致は後続 Issue / F019。
...
プログラムインタフェースのリソース境界・パス・誤り識別子は `openapi.yaml`（F006）を正とする。JSON フィールドの最終集合は実装 Issue で足す。
